...
def main(argv, wayout):
	...
	parser.add_argument('-Q', '--query_range', action="store_true", help="use start and end of query sequence in gff")
	...
	for line in open(args.blast, 'r'):
		...
		if args.augustus:
			...
			attributes += f";OrthologID={sseqid}"
		...
		if istart <= iend:
			strand = "+"
			outlist = [sseqid, args.program, args.type, start, end, bitscore, strand, ".", attributes]
			plusstrand += 1
		else:
			strand = "-"
			outlist = [sseqid, args.program, args.type, end, start, bitscore, strand, ".", attributes]
			minusstrand += 1
